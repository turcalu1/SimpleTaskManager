...
class DataController: NSObject {
    ...
    func preloadData () {
        ...
        for (i,c) in Common.DEFAULT_CATEGORIES.enumerate() {
            let category = CategoryPersistencyManager.sharedInstance.createCategory(i, name: c)
            categories.append(category)
        }
        
        let today = NSDate()
        for (i,t) in Common.DEFAULT_TASKS.enumerate() {
            TaskPersistencyManager.sharedInstance.createTask(t, due: Common.addDayAndHourToDate(i, hour: 0, date: today), category: categories[i])
        }
    }
}